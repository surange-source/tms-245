...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(923040100); //階段一
    ...
    map = eim.setInstanceMap(923040200);//階段二
    map.resetFully();
    map.resetSpawnLevel(level);
    map.setSpawns(false);
    map.killAllMonsters(false);
    ...
    map = eim.setInstanceMap(923040300);//階段三
    map.resetFully();
    map.resetSpawnLevel(level);
    ...
    map = eim.setInstanceMap(923040400);//階段四
    map.resetFully();
    ...
    switch (itemID) {
        case 2430364:
            ...
            if (eim.getProperty("KentaSave").equals("2")) {
                player.dropTopMsg("收集到了" + count + "個空氣玲。");
                if (count >= 10) {
                    for (i = 0; i < eim.getPlayers().size(); i++) {
                        eim.getPlayers().get(i).removeAll(2430364);
                    }
                    eim.getMapInstance(0).startMapEffect("咻，真是差點就要出大事了。現在氧氣充足，我們快到安全的地方去吧。", 5120052);
...
function waterLevelUp(eim) {
    ...
    if (level <= 17) {
        ...
        eim.getMapInstance(2).startMapEffect("哎，我真的很討厭蝙蝠。如果使用那邊的槓桿，就能前往上面的出口。", 5120052);
    }
}
...
function checkDone(eim) {
    ...
    if (pop > 0 && count >= eim.getMapInstance(2).getCharactersSize()) {
        ...
        eim.getMapInstance(2).startMapEffect("洞窟裡正在進水！在怪物們進來之前，我們快上去吧！！！", 5120052);
