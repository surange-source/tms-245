...
function start(mode, type, selection) {
    ...
    qm.sendNextPrev("他們是天生的劍士！從出生的那一刻起，對食物就充滿了無窮的憤怒，凡是他們經過的地方都不會留下任何食物。很可怕吧？");
    } else if (status == 5) {
    qm.sendNextPrevS("#b(他不是在開玩笑吧？)#k", 2);
    } else if (status == 6) {
    qm.askAcceptDecline("來，快點#b再次進入修煉場#k，去和那些天生的劍士們修煉用的豬中戰鬥吧，打倒#r30只#k後，你的能力將會有一個質的飛躍。全力以赴地去戰鬥吧！超越我這個老師！");
    } else if (status == 7) {
    qm.forceStartQuest();
...
function end(mode, type, selection) {
    ...
    qm.TutInstructionalBalloon("Effect/BasicEff.img/AranGetSkill");
    } else if (status == 4) {
