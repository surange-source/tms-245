...
function action(mode, type, selection) {
    if (status >= 27 && mode == 0) {
        cm.sendNext("如果你想到其他村子去，歡迎隨時來乘坐出租車～");
        cm.dispose();
        return;
    ...
    if (mode == 1)
        ...
        status--;
    if (status == 0) {
        cm.sendNext("你好～！我是#p1002000#。你想快速安全地移動到其他村莊嗎？那樣的話，請乘坐為顧客提供最好服務的#b#p1002000##k吧。我們可以以很便宜的價格，將你送到想去的地方。");
    } else if (status == 1) {
        cm.sendSimple("好的,選擇您想要進行的項目吧。\r\n#L0##b告訴我村莊的故事#l\r\n#L1#我想前往村莊#k#l");
    } else if (status == 2) {
        if (selection == 0) {
            cm.sendSimple("選擇您想要瞭解的村莊:\r\n#b#L0##m104000000##l\r\n#L1##m102000000##l\r\n#L2##m101000000##l\r\n#L3##m100000000##l\r\n#L4##m103000000##l\r\n#L5##m400000##l\r\n#L6##m105040300##l");
        } else if (selection == 1) {
            status = 26;
            if (cm.getJob() == 0) {
                var selStr = "新手玩家可以獲得#b90%#k的折扣。請選擇目的地。#b";
                for (var i = 0; i < maps.length; i++) {
                    selStr += "\r\n#L" + i + "##m" + maps[i] + "# (" + costBeginner[i] + " mesos)#l";
                }
            } else {
                var selStr = "請選擇你的目的地吧。按照目的地不同，車費也有所不同。#b";
                for (var i = 0; i < maps.length; i++) {
