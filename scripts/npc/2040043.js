...
function boxStage(cm) {
    ...
    if (cm.isLeader()) { // leader
        if (status == 0) {
            ...
            if (preamble == null) {
                cm.sendNext("歡迎來到5階段，在旁邊，有9個箱子。需要回答我的問題,並3個隊友站在箱子上組成正確答案，再由隊長來點擊我，看是否正確。");
                ...
                if (complete != null) {
                    ...
                    if (totplayers == 3 || debug) {
                        ...
                        if (debug) {
                            ...
                            cm.sendNext("需要3名隊員站在箱子上組成答案。只允許3名隊員站在箱子上, 你們的問題是:" + stage8question[parseInt(eim.getProperty("stage" + nthtext + "combo"))]);
                            cm.dispose();
                        }
                    }
                }
