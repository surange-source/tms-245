...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) { 
            ...
            if(cm.getBossLog("樂豆點突破1",1) <= 20){
            if(cm.haveItem(4000463, 100)){
            if (cm.changeLimitBreak(500000)) {
                cm.setBossLog("樂豆點突破1",1);
                cm.gainItem(4000463,-100);
                    cm.sendOk("#b傷害上限突破成功.\r\n\r\n本次追加傷害為：#r10000#b.");
                cm.worldSpouseMessage(0x20,"[傷害突破] 玩家 "+ cm.getChar().getName() +" 使用 中介幣 讓武器傷害上限突破成功 本次追加 500000 傷害值。");
            }else{
                    cm.sendOk("#b突破失敗.\r\n系統為檢測到角色身上裝備武器.");
            }
                    cm.dispose();
