...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) { 
            cm.sendSimple("#fUI/UIWindow2.img/Quest/quest_info/summary_icon/summary#\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#親愛的#r#h ##k您好，我是新手成長系統:\r\n#r#L1##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#新手成長系統簡介#l\r\n\r\n   #k#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#今天在線：#r"+cm.getOnlineTime()+"#k 分鐘#b\r\n\r\n#fUI/UIWindow2.img/QuestGuide/Button/WorldMapQuestToggle/normal/0#\r\n#L4##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必須]#b新手出生禮包領取#l\r\n#L100##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必須]#b領取10-250級等級獎勵#l\r\n#L3##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必須]#b免費領取BOSS競技場徽章一枚#l\r\n#L6##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必須]#b免費耀眼藍寶石勳章一枚(25星)#l\r\n\r\n#rPs:影武必須先戴短劍在戴刀注意武器能力值喔#k");
        } else if (status == 1) {
            if (selection == 1) {
            ...
            if((cm.getPlayer().getLevel() > 9 && cm.getPlayer().getLevel() < 30) && cm.getPQLog("引導10",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
                ...
                cm.sendOk("完成了傳奇新手成長系統階段功能 系統給予大量獎勵。");
                cm.dispose();
            } else if((cm.getPlayer().getLevel() > 99 && cm.getPlayer().getLevel() < 148)  && cm.getPQLog("引導100",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
                ...
                cm.sendOk("完成了傳奇新手成長系統階段功能 系統給予大量獎勵。");
                cm.dispose();
            } else if((cm.getPlayer().getLevel() > 149  && cm.getPlayer().getLevel() < 250)  &&  cm.getPQLog("引導150",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
                if(cm.getJob() == 112 || cm.getJob() == 122 || cm.getJob() == 132 || cm.getJob() == 5112){ //-- 劍士
                ...
                }else if(cm.getJob() == 412 || cm.getJob() == 434 || cm.getJob() == 1412){ //-- 盜賊
        ...
        cm.dispose();
            ...
            if (cm.getPQLog("送裝備", 1) == 0) {
            if (!cm.haveItem(1190400)) {    
            cm.gainItem(1190400,1);
            //cm.setPQLog("送裝備", 1);
            ...
            cm.dispose();
            cm.openNpc(9000281,"xrlb");
        } else if (selection == 6) {
            if (cm.getPQLog("糖服冒險家", 1) == 0) {
            if (!cm.haveItem(1142609)) {
            ...
            cm.sendOk("超強裝備已經給您發放.感謝您的支持.");
                ...
                cm.sendOk("您身上已經有#v1142609#了");
            }
            }
            cm.dispose();
        } else if (selection == 100) {
            cm.dispose();
            cm.openNpc(9900005,"dengji1001");
        }
       }
      }
}