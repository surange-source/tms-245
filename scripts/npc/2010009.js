...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        choice = selection;
        if (selection == 0) {
            cm.sendNext("多個公會結合在一起成立的組織被稱為公會聯盟。我負責管理公會聯盟的有關事務。");
            cm.dispose();
        } else if (selection == 1) {
            cm.sendNext("想要成立公會聯盟的話，必須有2名公會長組成組隊。其中隊長會成為公會聯盟的盟主。");
        } else if(selection == 2) {
            if (!cm.isLeader()) {
                cm.sendNext("只有會長可以創建公會聯盟。"); //Not real text
                cm.dispose();
            } else if (cm.getPlayer().getParty() == null || partymembers == null || partymembers.size() != 2) {
                cm.sendNext("必須要有2名公會長組成組隊。"); //Not real text
                cm.dispose();
        ...
        if (choice == 1) {
            cm.sendNext("2名公會長組隊之後，需要5000萬楓幣。這是創建公會聯盟所必需的手續費。");
        } else if (choice == 2) {
        ...
        if (choice == 1) {
            cm.sendNext("另外還有一個！如果已經加入其他公會聯盟的話，就無法建立新的公會聯盟！");
            cm.dispose();
        } else {
            guildName = cm.getText();
            cm.sendYesNo("你確定使用 #b"+ guildName + "#k 做為公會聯盟的名稱嗎？");
        }
    } else if (status == 4) {
            if (!cm.createAlliance(guildName)) {
                cm.sendNext("你不能使用這個名稱"); //Not real text
                status = 1;
                choice = 2;
            } else
                cm.sendOk("你已成功建立了公會聯盟。");
            cm.dispose();
    }
}
