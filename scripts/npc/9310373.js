...
var List = Array(
        Array(wi1 + "#e[回到市場]", 2, 1),//名稱、NPC代碼、模式
        Array(wi1 + "[每日跑環]", 9010009, null),
        Array(wi1 + "[萬能傳送]", 9310362, "chuansong"),
        Array(wi2 + "#r[VIP 商城]", 9310362, "vipshop"),
        Array(wi2 + "[在線獎勵]", 9310362, "zaixian2"),
        Array(wi2 + "[神器提升]", 9310362, "mrfl2"),
        Array(wi3 + "#d[等級獎勵]", 9310362, "dengji1001"),
        Array(wi3 + "[管理中心]", 9000030, null),
        Array(wi3 + "[BOSS重置]", 9310362, "BOSScz2"),
      Array(wi4 +"#g[刪除道具]", 9310362, "Delete"),
      Array(wi4 +"[瑪瑙系統]", 9130115, "manaoshengji2"),
      Array(wi4 +"[所有商店]", 9310362, "Allshop"),
      Array(vvv4 +"#b [幣種合成]", 9310362, "BZHC"),
      Array(vvv4 +"[裝備分解]", 9330603, null),
      Array(vvv4 +"[快速五轉]\r\n", 1540496, null)
        )
var text;
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (a == -1) {
            ...
            //text = cool+"";
            text = "" + tz10 + "在線時間：#r" + cm.getOnlineTime() + "#k分\t  " + tz13 + "餘額：#r" +cm.getHyPay(1)+ "#k\t" +tz12+"樂豆點：#r" + cm.getNX(1) + "#k\r\n" + tz8 + "楓點：#r" + cm.getNX(2) + "#k\t " +  axx1 + "里程：#r" +cm.getPlayerPoints()+ "#k\t\t"+ epp3+"活躍：#r"+cm.getPlayerEnergy()+"#k\r\n\r\n";
