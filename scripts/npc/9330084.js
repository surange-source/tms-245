...
function action(mode, type, selection) {
    ...
    if (p.getEventPoints(eventid, cm.getPlayer().getId()) >= 1) {
        ...
        text += "#L3##r" + eff + " 隨機加入門派！（有勇敢獎勵哦！）#l\r\n ";
        cm.sendSimple(text)
    } else if (status == 1) {
        if (selection == 3) { // 隨機加入的
            cm.sendNext("這項操作將隨機加入到一個已有的門派裡面，你確定嗎？");
        } else {
            p = new EvPoint();
            if (p.getEventPoints(eventid, cm.getPlayer().getId()) >= 1) {
