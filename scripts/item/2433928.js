...
function action(mode, type, selection) {
 ...
 if (status === 0) {
     ...
     var slot = 0 < slotType && slotType < 5 ? im.getInventory(slotType).getSlotLimit() : (slotType === 0 ? im.getPlayer().getTrunk().getSlots() : 128);
     switch (slotType) {
         case 0:
             sSlot = "倉庫";
             break;
     ...
     if (im.used()) {
         ...
         switch (slotType) {
             case 0:
                im.getPlayer().getTrunk().increaseSlots(nSlot);
                im.getPlayer().getTrunk().saveToDB();
                slot = im.getPlayer().getTrunk().getSlots();
                break;
             case 1:
             case 2:
             case 3:
