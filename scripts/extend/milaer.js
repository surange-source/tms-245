...
function action(mode, type, selection) {
    ...
    if (cm.getPlayer().getClient().getChannel() == 1 || cm.getPlayer().getClient().getChannel() == 2 || cm.getPlayer().getClient().getChannel() == 3) {
        if (status == 0) {
            cm.sendSimple("- #e#d米納爾森林保衛戰#k#n:\r\n\r\n#b副本一共提供七個關卡,每個關卡怪物血量在3000萬-5000萬之間，每消滅一個關卡的怪物即可進入下一個關卡，最後的關卡等待的中的BOSS，消滅後會解救出被綁架的NPC，同時瘋狂點擊該NPC會獲得獎勵，獎勵請到市場彪魯處兌換。#k\r\n副本要求：\r\n#r1). 可在1,2,3線可挑戰。\r\n2). 組隊員等級必須要在" + minLevel + "級以上。\r\n3). 組隊員必須要" + minPartySize + "人以上，" + maxPartySize + "人以下。\r\n\r\n#L0#[執行]米納爾森林保衛戰#l")
        } else if (status == 1) {
            if (selection == 0) {
                    ...
                    if (next) {
                        ...
                        if (em == null) {
                            ...
                            if (prop.equals("0") || prop == null) {
                                ...
                                cm.setPQLog("海怪");
                                cm.worldSpouseMessage(0x20,"[米納爾森林保衛戰] ：玩家 "+ cm.getChar().getName() +" 進入了米納爾森林保衛戰。");
                                cm.dispose();
                                return;
                            } else {
