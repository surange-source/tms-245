...
var mhair2 = Array(38520,35750,35730,35721,35712,35705,35693,38640,35550,38032,38264,36490,38100,38020,38000,37990,38051,38040,35152,38123,35084,35063,35211,38440,38297,38481,38380,38240,38220,38363,38370,35360,35330,38600,38680,38590,38560,38570,38490,38460,38610,38580,35460,35440,35430,35560,38770,38690,38700,35602,38627,36340,35090,35080,35160,35150,35130,35120,35110,35100,35190,35240,35230,35220,35350,36130,36160,36140,36170,36480,36470,36460,36450,36620,36630,36640,36650,36670,36690,36710,36730,36740,36750,36760,36770,36800,36820,36840,36860,36890,36900,36920,36930,36980,36940,36990,33380,33750,33790,34370,34940,35000,35010,35020,35030,35040,35050,35070,37690,37890,31870,32470,32490,33645,35060,35180,35200,35210,35260,35340,36230,36680,36490,36810,36830,36880,36950,37120,36700);
...
var hairnew = Array();
var mface = Array(20098, 23059, 24057, 23053, 21099, 23057,  23033, 23034, 23029, 23030, 24025, 24015, 23025, 23019, 23831, 23014, 23024, 23017, 23015, 23110, 23021, 23016, 24011, 24012, 20099, 24001, 23004, 21092, 20000, 20001, 20002, 20003, 20004, 20005, 20006, 20007, 20008, 20009, 20010, 20011, 20012, 20013, 20014, 20015, 20016, 20017, 20018, 20019, 20020, 20021, 20022, 20023, 20024, 20025, 20026, 20027, 20028, 20029, 20036, 20045, 20037, 20040, 20048, 20049, 20050, 20051, 20052, 20053, 20055, 20056, 20058, 20059, 20060, 20062, 20063, 20064, 20065, 20066, 20068, 20067, 20069);
var fface = Array(21000, 21001, 21002, 21003, 21004, 21005, 21006, 21007, 21008, 21009, 21010, 21011, 21012, 21013, 21014, 21015, 21016, 21017, 21018, 21019, 21020, 21021, 21022, 21023, 21024, 21025, 21026, 21027, 21034, 21043, 21029, 21030, 21038, 21046, 21047, 21048, 21049, 21050, 21052, 21053, 21054, 21055, 21056, 21057, 21058, 21059, 21060, 21061, 21062, 21063, 21064);
var facenew = Array();
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.sendSimple("你現在選擇的是 #r美容美發#k 還能什麼開始打造自己吧!\r\n已經更新#rver123#k所有臉型和髮型,普通沒有的,都在皇家裡面\r\n" + bbb + " 賬號目前 (#k贊助金額： #r"+cm.getHyPay(1)+"元#k)\r\n" + bbb + " 你有: #r" + cm.itemQuantity(5150040) + " #k張#b<#z5150040#>#k" + bbb + " 你有: #r" + cm.itemQuantity(5152053) + " #k張#b<#z5152053#>#k\r\n#L4#" + aaa + " #b皮膚染色（普通玩家會員專用免費）#l\r\n#L2#" + aaa + " 髮型染色（普通玩家會員專用免費）#l\r\n#L0#" + aaa + " 改變臉型（普通玩家會員專用免費）#l\r\n#L1#" + aaa + " 眼睛染色（普通玩家會員專用免費）#l\r\n#L3#" + aaa + " 普通髮型一（普通玩家會員專用免費）#l\r\n#L5#" + aaa + " 普通髮型二（普通玩家會員專用免費）#l\r\n#L8#" + aaa + " #r貴族124最新髮型（VIP月卡免費理發）#l\r\n#L6#" + aaa + " #r皇家髮型（需要消耗一張#z5150040#）#l\r\n#L7#" + aaa + " 皇家臉型（需要消耗一張#z5152053#）#l\r\n#L18#" + aaa + " 機器人整容#k（永不變心的陪伴你#n）#l");
        } else if (status == 1) {
            if (selection == 0) {
                facenew = Array();
                ...
                cm.sendStyle("我可以改變你的臉型,讓它比現在看起來漂亮. 你為什麼不試著改變它下? ,我將會幫你改變你的臉型,那麼選擇一個你想要的新臉型吧!", facenew, 5152001);
            } else if (selection == 1) {
                beauty = 1;
                if (cm.getChar().getGender() == 0) {
                ...
                cm.sendStyle("請選擇你喜歡的顏色.", colors, 5152001);
            } else if (selection == 2) {
                beauty = 2;
                haircolor = Array();
                ...
                cm.sendStyle("我可以改變你的髮色,讓它比現在看起來漂亮. 你為什麼不試著改變它下? 我將會幫你改變你的髮色,那麼選擇一個你想要的新髮色吧!", haircolor, 5151001);
                ...
                cm.sendStyle("我可以改變你的髮型,讓它比現在看起來漂亮.你為什麼不試著改變它下? 我將會幫你改變你的髮型,那麼選擇一個你想要的新髮型吧!", hairnew, 5150001);
            } else if (selection == 4) {
                beauty = 4;
                cm.sendStyle("用我們特殊開發的機器可查看護膚後的效果噢,想換成什麼樣的皮膚呢？請選擇～~", skin, 5153000);
            } else if (selection == 6) {
                    cm.dispose();
                    cm.openNpc(1012117);
            } else if (selection == 7) {
            ...
            if (beauty == 0) {
                ...
                if (cm.haveItem(4032521) == 0) {
                    cm.sendOk("你沒有#v4032521#贊助福利卡");
                    cm.dispose();
                } else if (cm.haveItem(4032521) == 1) {
                    cm.setHair(hairnew[selection]);
                    cm.sendOk("好了,你的朋友們一定認不出來是你了!");
                    cm.dispose();
                } else if (cm.getPlayer().getCSPoints(1) >= 0) {
