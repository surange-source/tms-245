...
function action(mode, type, selection) {
    ...
    if (status > 2) {
        if(section == 0) {
        cm.sendOk("請仔細想想。一旦你做了決定，讓我知道。");
        } else {
        cm.sendOk("想一想，然後讓我知道你的決定。");
        }
        cm.safeDispose();
    }
    status--;
    ...
    section = selection;
    if(section == 0) {
        cm.sendSimple("不像是我們村子的人，找我什麼事情？\r\n#L0##b想得到魔法種子#k#l");
    } else if (section == 1) {
    ...
    if(section == 0) {
        cm.sendGetNumber("#b魔法的種子#k是貴重物品，不能白送給你。付出一定的代價可以嗎?一個#b魔法的種子#k#b1000樂豆點#k買嗎?\r\n要幾個?",0,0,99);
    } else {
    ...
    if(section == 0) {
        if(cm.getHyPay(1) < cost || !cm.getPlayer().getCSPoints(1)) {
