...
function start() {
    ...
    if (info == "") {
        cm.sendYesNo("哎呀，要給我皇家整形券才能幫你處理頭髮喔。要現在移動到商城去看看現在賣哪些皇家整形券嗎？");
    } else {
        canRoyal = true;
...
function action(mode, type, selection) {
    ...
    if (status === i++) {
        ...
        if (royalCards.indexOf(royalCardID) != -1) {
            cm.sendYesNo("你帶來#b#t" + royalCardID + "##k了！我目前還在受訓的階段，不知道能不能讓你滿意，但你要不要使用#b#t" + royalCardID + "##k來整形呢？");
        } else {
            cm.sendOk("發生未知錯誤");
            cm.dispose();
            return;
        ...
        if (!cm.haveItem(royalCardID)) {
            cm.sendNext("必須要有#b#t" + royalCardID + "##k，我才能為你整形。");
            cm.dispose();
            return;
        }
        ...
        var nStyle = gStyle != null ? (parseInt(gStyle.getItemId() / 1000) * 1000 + gStyle.getItemId() % 100 + parseInt((isSecond ? cm.getPlayer().getSecondFace() : cm.getPlayer().getFace()) % 1000 / 100)) : 0;
        ...
        if (isZero && selection == 2) {
            ...
            nStyle = gStyle != null ? (parseInt(gStyle.getItemId() / 1000) * 1000 + gStyle.getItemId() % 100 + parseInt(cm.getPlayer().getSecondFace() % 1000 / 100)) : 0;
            if (gStyle == null || !cm.canChangeFace(nStyle)) {
                cm.sendOk("發生未知錯誤");
                cm.gainItem(royalCardID, 1);
                cm.dispose();
