...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        for (var i = 0;i<potline.length ;i++ ){
            TXT +="#L"+(i+1)+"##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#[一鍵潛能]#b裝備潛能第"+(i+1)+"條屬性#k  (#r火爆內容#k)#v"+potline[i][1]+"##l\r\n"; 
        }
        cm.sendSimple(TXT);
    } else if (status == 1) {
        A = selection;
        ...
        var selStr = "#fUI/UIWindow2.img/Quest/quest_info/summary_icon/summary#\r\n\r\n親愛的#r#h ##k您好!想選擇什麼樣的潛能:\r\n#k擁有#z"+potline[A-1][1]+"#: #r" + cm.itemQuantity(potline[A-1][1]) + "#v"+potline[A-1][1]+"##k\r\n\r\n";
        for (var i = 0; i < potList.length; i++) {
            selStr += "\r\n#L" + i + "##b " + potList[i][1] + "#k  \r\n\t#d(需消耗： #r" + potList[i][2] + "#k #k#d #z"+potline[A-1][1]+"#)#l\r\n";
        }
        cm.sendSimple(selStr + " \r\n ");
    } else if (status == 2) {
        oldEquip = cm.getEquipBySlot(slot);
        if (oldEquip == null ) { //oldEquip.getState() != 0x14 檢測裝備普通潛能是否 SS
        ...
        if (pot != null) {
            ...
            depict = pot[1];
            points = pot[2];
            cm.sendYesNo("您是否要將#b裝備欄第一格裝備#k\r\n第#r["+potline[A-1][0]+"]#k條潛能屬性(不會改變其它潛能屬性)#k\r\n一鍵洗潛能屬性為 #b" + depict + "#k？");
        } else {
            cm.sendOk("出現錯誤...");
            cm.dispose();
        }
    } else if (status == 3) {
    if(cm.itemQuantity(potline[A-1][1]) < 1) {
        cm.sendOk("請確認背包中是否存在#v"+potline[A-1][1]+"##t"+potline[A-1][1]+"#");
        cm.dispose();
        return;
    }
        if (potId <= 0 || depict == "" || points <= 0) {
            ...
            cm.sendYesNo("洗出潛能屬性為 #b" + depict + "#k 用了 #b" + count + "#k 次。\r\n本次保留該屬性需要:#r 1 #k #r#z"+potline[A-1][1]+"##k 是否保留該屬性？");
    } else if (status == 4) {
        newEquip = cm.getEquipBySlot(slot);
        if (oldEquip == newEquip) { //這個地方還需要檢測樂豆點數量
                if (cm.changePotential(slot, potline[A-1][0], potId, true)) { //[裝備位置] [潛能位置] [潛能ID] [是否公告]
                    //todo 扣樂豆點
            cm.gainItem(potline[A-1][1],-1);
