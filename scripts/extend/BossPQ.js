...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        switch (selection) {
            ...
            case 1:
            case 2:
                cm.sendGetNumber("How many " + (selection == 1 ? "Enchanted Scrolls" : "Bamboo Luck Sacks") + " 你想進行交換麼?\r\n", cm.itemQuantity(selection == 1 ? 5221001 : 3993002), 1, 100);
                break;
            case 3:
            case 4:
            case 5:
        ...
        if (select == 1 || select == 2) {
            if (cm.haveItem(itemid, selection)) {
                ...
                cm.sendOk("請檢查你是否有足夠的交換物品，或者沒有足夠的空間.");
            }
...
function chooseItem(index) {
    var choice = "請選擇下面項目中的任意一項:#b";
    for (var i = 0; i < items[index].length; i++)
        choice += "\r\n#L" + i + "##i" + items[index][i][0] + "# 需要 " + items[index][i][1] + " BOSS組隊點 (#z" + items[index][i][0] + "#)#l";
    choice += "\r\n ";
...
function gainReward(intPoints, record, index) {
    if (intPoints >= items[index][sel][1] && cm.canHold(items[index][sel][0])) {
        ...
        cm.sendOk("請檢查你是否有足夠的交換物品，或者沒有足夠的空間.. #b當前擁有 : " + points);
    }
}
