...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == 180000001) {
        ...
        selStr += "#L0#" + eff + "#b查看#k#r50#k   #b購買禮包        [內大量道具+#k#r15#k#b萬樂豆點]#k#l\r\n";
        selStr += "#L1#" + eff + "#b查看#k#r100#k  #b購買禮包        [內大量道具+#k#r30#k#b萬樂豆點]#k#l\r\n";
        selStr += "#L2#" + eff + "#b查看#k#r500#k  #b購買禮包        [內大量道具+#k#r150#k#b萬樂豆點]#k#l\r\n";
        selStr += "#L3#" + eff + "#b查看#k#r1000#k #b購買禮包        [內大量道具+#k#r300#k#b萬樂豆點]#k#l\r\n";
        selStr += "#L4#" + eff + "#b查看#k#r3000#k #b購買禮包        [內大量道具+#k#r600#k#b萬樂豆點]#k#l\r\n";
        selStr += "#L5#" + eff + "#b查看#k#r6000#k #b購買禮包        [內大量道具+#k#r1800#k#b萬樂豆點]#k#l\r\n";
        cm.sendSimple(selStr);
    } else if (status == 1) {
        switch (selection) {
        case 0:
        if(cm.getEventCount("50贊助") == 0){
           if(cm.getHyPay(1) < 50 && (cm.getSpace(1) < 4||cm.getSpace(2) < 4||cm.getSpace(3) < 4||cm.getSpace(4) < 4)){
        ...
        if(cm.getEventCount("100贊助") == 0){
           if(cm.getHyPay(1) < 100 && (cm.getSpace(1) < 9||cm.getSpace(2) < 9 ||cm.getSpace(3) < 9 ||cm.getSpace(4) < 9 )){
        ...
        if(cm.getEventCount("500贊助") == 0){
           if(cm.getHyPay(1) < 500 && (cm.getSpace(1) < 12||cm.getSpace(2) < 12 ||cm.getSpace(3) < 12 ||cm.getSpace(4) < 12 )){
            cm.sendOk("請確認賬號餘額是否達到500贊助金額\r\n請確認裝備背包欄至少空出12格以上位置\r\n請確認消耗背包欄至少空出12格以上位置\r\n請確認其它背包欄至少空出12格以上位置\r\n請確認設置背包欄至少空出12格以上位置");
                cm.dispose();
           }else if(cm.addHyPay(500) > 0){
                cm.gainNX(1500000);
                cm.gainItem(2431987,4);//週年慶50%卷軸箱子 代碼測試
                cm.gainItem(2049124,25);//正向混沌卷軸X5
                cm.gainItem(2049116,25);//強化混沌卷軸X5
                cm.gainItem(2049313,5);//5星裝備強化卷軸X1
                cm.gainItem(2049752,2);//S級潛能卷軸 30% X3
                cm.gainItem(3010659,1);//巴士車站椅子 X1
                cm.gainItem(3010608,1);//完美的名畫椅子X1
                cm.gainItem(3010590,1);//酸甜點心椅子X1
                cm.gainItem(1142179,1);//王座收藏家X1
                cm.gainItem(1112915,3);//藍調戒指X3
                cm.gainItem(2431938,1);//150隨機裝備箱子X1
        ...
        if(cm.getEventCount("1000贊助") == 0){
           if(cm.getHyPay(1) < 1000 && (cm.getSpace(1) < 15||cm.getSpace(2) < 15 ||cm.getSpace(3) < 15 ||cm.getSpace(4) < 15 )){
        ...
        if(cm.getEventCount("3000贊助") == 0){
           if(cm.getHyPay(1) < 3000 && (cm.getSpace(1) < 23||cm.getSpace(2) < 23 ||cm.getSpace(3) < 23 ||cm.getSpace(4) < 23 )){
        ...
        if(cm.getEventCount("6000贊助") == 0){
           if(cm.getHyPay(1) < 6000 && (cm.getSpace(1) < 23||cm.getSpace(2) < 23 ||cm.getSpace(3) < 23 ||cm.getSpace(4) < 23 )){
            cm.sendOk("請確認賬號餘額是否達到3000贊助金額\r\n請確認裝備背包欄至少空出23格以上位置\r\n請確認消耗背包欄至少空出23格以上位置\r\n請確認其它背包欄至少空出23格以上位置\r\n請確認設置背包欄至少空出23格以上位置");
                cm.dispose();
                ...
                cm.setEventCount("6000贊助");
                 cm.sendOk("請注意查收。");
            cm.worldSpouseMessage(0x20,"[贊助禮包] 玩家 "+ cm.getChar().getName() +" 領取了6000贊助禮包。");
                cm.dispose();
             }else{
                 cm.sendOk("儲值餘額不足,請儲值後領取！");
                cm.dispose();
