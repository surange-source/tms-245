...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
                        var selStr = "#d您好，活動期間只要收集16枚春節幣即可領取活動獎勵。#n#k\r\n";
                        selStr +="\r\n#L1##b"+aaa+" 16枚春節幣兌換2x週年慶武器卷軸禮包#l#k\r\n";
                        cm.sendSimple(selStr);    
        } else if (status == 1) {
            if (selection == 1) {
                typed=1;
                cm.sendYesNo("- #e#d活動獎品：#n#k\r\n\r\n#b需要#t4310110# x 16：\t\t\t\t #r " + cm.itemQuantity(4310110) + " / 16 個\r\n\r\n當您收集完後將會得到\r\n #i2431987# 週年慶50%武器攻擊卷軸禮包 x2 個\r\n\r\n- #e#d管理提示：#n#b點是進行領取。點否返回上一頁.#k");
            } else if (selection == 2) {
                typed=2;
            ...
            if(typed==1){
                if (cm.haveItem(4310110, 16)) {
            cm.gainItem(4310110, -16);
