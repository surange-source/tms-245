/* 
 *  日常任務
 *  任務初始ID 200106，子任務依次加1，新任務分類的初始ID需間隔100，如100200
 */
...
var maplist = Array(
        ...
        //Array(701010321, "東方神州 - 黑羊的領土"),
        //Array(701010322, "東方神州 - 通道"),
        //Array(701010323, "東方神州 - 危險的山丘"),
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == -1) {
            ...
            accepttimes = maxtimes - cm.getPQLog("尋寶任務");
            text = "\t\t\t#e每日尋寶 - 里諾赫的黃金袋子#n\r\n\r\n";
            text += "我的財富，遍佈世界各地……\r\n";
            text += "今日剩餘尋寶次數：#r" + accepttimes + "#k 次\r\n";
            text += "#r#L999#尋寶任務簡介#l#k\r\n\r\n";
            if (cm.getPlayer().getTreasureQid() > 0) {  // 判斷是否接取了任務
                if (cm.isQuestFinished(cm.getPlayer().getTreasureQid())) { // 判斷是否完成任務
            ...
            if (selection == 999) {
                ...
                cm.sendYesNo("是否要花費#r800#k點購買一個黃金羅盤？");
                ...
                text = "寶藏有可能藏匿在一下幾個地圖，你有#b10分鐘#k的時間尋找的正確地圖並打開我的黃金袋子！\r\n";
                for (var key in lastMapList) {
                    qinfo += "#b" + lastMapList[key][1] + "\r\n";
                    text += "#b" + lastMapList[key][1] + "\r\n";
                }
                spawnNpc(mapid);
                text += "#r#e提示我只給你一次哦，一定要記牢！#n#k";
                // 重新接受任務
            ...
            if (cm.getPlayer().getCSPoints(1) >= 80)
            {
                if (cm.getSpace(2) < 1) {
                    cm.sendOk("消耗欄不足");
                    cm.dispose();
