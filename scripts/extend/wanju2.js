...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        for (var i = 0; i < itemList.length; i++) {
            selStr += "\r\n#L" + i + "##i" + itemList[i][0] + ":# #b#t" + itemList[i][0] + "##k x(" + itemList[i][1] + ") #r" + itemList[i][2] + "#b個#v4001266##l";
        }
        ...
        if (item != null) {
            ...
            selequantity = item[1];
            selectedCost = item[2];
            cm.sendYesNo("您是否兌換#i" + selectedItem + ":# #b#t" + selectedItem + "##k x (" + selequantity + ") 需要 #r" + selectedCost + "#k #v4001266#？");
        } else {
            cm.sendOk("出現錯誤...");
            cm.dispose();
        }
        ...
        if (cm.getItemQuantity(4001266) >= selectedCost) {
            if (cm.canHold(selectedItem, selequantity)) {
                ...
                cm.worldMessage("『玩具塔兌換』 " + cm.getName() + " 玩家使用勞動獎章兌換了： " + cm.getItemName(selectedItem) + " x " + selequantity);
       ...
       if (cm.canHold(selectedItem, selequantity)) {
            ...
            cm.sendOk("您沒有足夠的#v4001266#。\r\n\r\n兌換#i" + selectedItem + ":# #b#t" + selectedItem + "##k x (" + selequantity + ") 需要 #r" + selectedCost + "#k #v4001266#。");
        }
        status = -1;
    }
}