...
public class SearchGenerator {
    ...
    public static Map<Integer, String> getSearchs(SearchType type) {
        ...
        switch (type) {
            case 道具:
                values.putAll(MapleItemInformationProvider.getInstance().getAllItemNames());
                break;
            case NPC:
                values.putAll(MapleLifeFactory.getNpcNames());
                break;
            case 地圖:
                for (int mapid : MapleMapFactory.getMapNames().keySet()) {
                    values.put(mapid, "'" + MapleMapFactory.getMapStreetName(mapid) + " : " + MapleMapFactory.getMapName(mapid) + "'");
                }
                break;
            case 怪物:
                values.putAll(MapleLifeFactory.getMonsterNames());
                break;
            case 任務:
