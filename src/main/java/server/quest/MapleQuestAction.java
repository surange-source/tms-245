    ...
    public void runStart(MapleCharacter chr, Integer extSelection) {
        ...
        switch (type) {
            case exp:
                ...
                chr.gainExp(intStore * GameConstants.getExpRate_Quest(chr.getLevel()) * ((chr.getTrait(MapleTraitType.sense).getLevel() * 3 / 10) + 100) / 100, true, true, true);
                break;
            case item:
...
//		        MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    public void runEnd(MapleCharacter chr, Integer extSelection) {
        switch (type) {
            case exp: {
                chr.gainExp(intStore * GameConstants.getExpRate_Quest(chr.getLevel()) * (chr.getStat().getQuestBonus()) * ((chr.getTrait(MapleTraitType.sense).getLevel() * 3 / 10) + 100) / 100, true, true, true);
                break;
            }
            case item: {
                // first check for randomness in item selection
