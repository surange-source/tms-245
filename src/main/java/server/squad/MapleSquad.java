...
public class MapleSquad {
    ...
    private final Map<String, String> members = new LinkedHashMap<>(); //[角色名字] [職業類型]
    ...
    public MapleSquad(int ch, String type, MapleCharacter leader, int expiration, String toSay) {
        ...
        this.ch = ch; //遠征隊所在頻道
        this.toSay = toSay; //廣告提示
        this.type = MapleSquadType.valueOf(type.toLowerCase());
        this.status = 1; //遠征隊的狀態
        this.beginMapId = leader.getMapId(); //開始遠征的地圖 也就是準備的地圖
    ...
    public void scheduleRemoval() {
        removal = EtcTimer.getInstance().schedule(new RemovalTask(), expiration);
    }
    ...
    public String getNextPlayer() {
        ...
        sb.append("#b").append(type.queue.get(ch).size()).append(" #k ").append("遠征隊成員信息 : \n\r ");
        int i = 0;
    ...
    public void acceptMember(int pos) {
        ...
        if (toadd != null && getChar(toadd) != null) {
            ...
            getChar(toadd).dropMessage(5, getLeaderName() + " 將你列為遠征隊隊員.");
        }
    }
    ...
    public void banMember(int pos) {
        ...
        if (toban != null && getChar(toban) != null) {
            ...
            getChar(toban).dropMessage(5, getLeaderName() + " 將你請出遠征隊，目前無法加入遠征隊.");
        }
    }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("總共 : ");
                sb.append("#b").append(members.size()).append(" #k ").append("個遠征隊成員 : \n\r ");
                int i = 0;
    ...
    private class RemovalTask implements Runnable {
        ...
        @Override
        public void run() {
            if (status != 0 && leader != null && (MapleSquad.this.getLeader() == null || status == 1)) { //leader itself = null means we're already cleared
