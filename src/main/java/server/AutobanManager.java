...
public class AutobanManager implements Runnable {
    ...
    private static final Logger log = LogManager.getLogger("AutobanManager");
    ...
    public void autoban(MapleClient c, String reason) {
        ...
        if (c.getPlayer().isGm()) {
            ...
            WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.spouseMessage(UserChatMessageType.藍加粉, "[GM消息] 玩家: " + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等級 " + c.getPlayer().getLevel() + ") 遊戲操作異常. (原因: " + reason + ")"));
        }
    }
